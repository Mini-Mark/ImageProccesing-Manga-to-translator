...
def detect(save_img=False):
    ...
    # if not os.path.exists(f"yolov7/runs/detect/{get_last_exp_folder('yolov7/runs/detect/')}/crops"):
    #     new_src = get_last_exp_folder('yolov7/runs/detect/').replace("exp","");
    #     os.mkdir(f"yolov7/runs/detect/exp{int(new_src)+1}")
    #     os.mkdir(f"yolov7/runs/detect/{get_last_exp_folder('yolov7/runs/detect/')}/crops")
    crp_cnt = 0
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            os.mkdir(f"yolov7/runs/detect/{get_last_exp_folder('yolov7/runs/detect/')}/crops")
            os.mkdir(f"yolov7/runs/detect/{get_last_exp_folder('yolov7/runs/detect/')}/crops/text_bubble")
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    #save crop part
                    crop_file_path = os.path.join(f"yolov7/runs/detect/{get_last_exp_folder('yolov7/runs/detect/')}/crops/text_bubble","test"+str(crp_cnt+1)+".jpg")
                    ...
                    if save_txt:  # Write to file
                        ...
                        with open(f'{txt_path}-predict.txt', 'a') as f:
                            f.write(('%g ' * len(line)).rstrip() % line + '\n')
                
                        with open(f'{txt_path}-crop-detail.txt', 'a') as f:
                            f.write(f"{crop_file_count}) top-left: ({x1},{y1}) | bottom-right: ({x2},{y2})\n")
