...
#     cv2.rectangle(image_show, top_left, bottom_right, (0, 255, 0), 2)
    ...
    with open(f"./{model_version}/runs/detect/{get_last_exp_folder(f'./{model_version}/runs/detect/')}/labels/test-crop-detail.txt", "r") as file:
    ...
    result_image = cv2.imread(
        f'./result/result_{get_last_exp_folder(f"./{model_version}/runs/detect/")}.jpg')

    if(model_version == "yolov5"):
        x, y = pos_top_left[0]-7, pos_top_left[1]-10
    elif(model_version == "yolov7"):
        x, y = pos_top_left[0], pos_top_left[1]+1
    ...
    cv2.imwrite(
        f'./result/result_{get_last_exp_folder(f"./{model_version}/runs/detect/")}.jpg', result_image)

# Get input from the user


def run(model, upload_path):

    global model_version

    model_version = model

    file_src = f'./{model_version}/runs/detect/{get_last_exp_folder(f"./{model_version}/runs/detect/")}/'
    shutil.copy(f'./{upload_path}/test.jpg',
                f'./result/result_{get_last_exp_folder(f"./{model_version}/runs/detect/")}.jpg')

    print("Translate")
    print(f"{file_src}crops/text_bubble/*.jpg")
    pos = 1
    ...
    return [f'./result/result_{get_last_exp_folder(f"./{model_version}/runs/detect/")}.jpg', f'./{model_version}/runs/detect/{get_last_exp_folder(f"./{model_version}/runs/detect/")}/test.jpg']
